"""Generate reproducible AP Toolbox H17 Pro calibration coefficients.
...
@njit(cache=True)
def ap_toolbox_h17_pro_action(cards: np.ndarray, count: int, dealer: int, can_split: bool, can_double: bool, can_surrender: bool, split_aces: bool, tc: int) -> int:
    """Exact policy encoded from the supplied AP Toolbox H17 Pro chart."""
...
def play_player(first: int, second: int, dealer: int, shoe: np.ndarray, position: int, running_count: int, policy: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, int, int, bool]:
    ...
    while index < hands:
        ...
        action = ap_toolbox_h17_pro_action(cards[index], count, dealer, can_split, count == 2, can_surrender, ace_split[index] == 1, tc)
...
def simulate_task(decks: int, dealt_decks: float, shoes: int, seed: int, policy: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    ...
    for _ in range(shoes):
        ...
        while position < cut_position:
            ...
            tc = int(math.floor((running_count * 52.0) / (len(shoe) - position + 1)))
            take_insurance = policy == AP_TOOLBOX_H17_PRO and dealer_up == 1 and tc >= 3
            if dealer_blackjack:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate CountLab AP Toolbox H17 Pro coefficients")
    ...
    parser.add_argument("--policy", choices=("ap-toolbox-h17-pro",), default="ap-toolbox-h17-pro")
    ...
    payload = {
        "metadata": {
            ...
            "strategy": "CountLab H17/DAS/RSA/LS basic strategy plus the supplied AP Toolbox H17 Pro 34-deviation chart, including insurance at TC +3",
            "model": "6/8 deck | H17 | DAS | RSA | LS | peek | 3:2 | one spot | AP Toolbox H17 Pro 34 deviations | insurance TC +3",
        }, "profiles": profiles,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    args.typescript.parent.mkdir(parents=True, exist_ok=True)
    write_typescript(payload, args.typescript, "AP_TOOLBOX_H17_PRO", Path(__file__).name)
