...
test("a saved scenario carries its inputs from the Lab into trip planning", async ({ page }) => {
  ...
  if (!await bankroll.isVisible()) await page.getByRole("tab", { name: "Bankroll" }).click();
  await bankroll.fill("12345");
  await page.getByRole("button", { name: "Save scenario" }).click();
  const dialog = page.getByRole("dialog", { name: "Save scenario" });
  await dialog.getByRole("textbox", { name: "Scenario name" }).fill("Weekend test");
  await dialog.getByRole("button", { name: "Save", exact: true }).click();
  await dialog.getByRole("link", { name: "Trip Planner" }).click();
