...
test("public pages and the footer fit a 320px screen", async ({ page }) => {
  ...
  await prepare(page, { guest: true });
  for (const path of ["/", "/analyze/", "/play/", "/dashboard/", "/settings/", "/training/basic-strategy/", "/training/running-count/", "/training/true-count/", "/training/deck-estimation/", "/training/benchmark/", "/reference/", "/reference/h17-chart/"]) {
    await page.goto(path);
