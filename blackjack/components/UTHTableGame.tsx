  ...
  return (
    <div className="mt-5 grid grid-cols-1 gap-5 pb-24 lg:pb-0 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-5">
        ...
        <CasinoTable>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--accent)]/55">Ultimate Texas Hold&apos;em</p><p className="mt-1 text-sm text-emerald-50/80">Bankroll <b className="text-white">${bankroll.toFixed(2)}</b></p></div><span className="rounded-full bg-well/20 px-3 py-1 text-xs text-[var(--accent)]/70">{phase === "betting" ? "Place bets" : phase === "result" ? "Round complete" : "Decision in progress"}</span></div>
      ...
      <div className="space-y-5">
        ...
        <Panel><h2 className="font-semibold">Session</h2><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Rounds</p><b className="text-xl">{stats.rounds}</b></div><div className="rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Wins</p><b className="text-xl">{stats.wins}</b></div><div className="col-span-2 rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Session net</p><b className={`text-xl ${stats.net >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}`}>{stats.net >= 0 ? "+" : ""}${stats.net.toFixed(2)}</b></div></div><div className="mt-4"><NumberField label="Reset bankroll" value={bankroll} min={1} prefix="$" disabled={locked} onValueChange={setBankroll} /></div></Panel>
