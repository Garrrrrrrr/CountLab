...
const CATALOGS: Record<Ruleset, { label: string; rows: typeof H17_PRO_DEVIATIONS }> = {
  h17: { label: "AP Toolbox H17 Pro", rows: H17_PRO_DEVIATIONS },
  s17: { label: "AP Toolbox S17 Pro", rows: S17_PRO_DEVIATIONS },
};
...
export default function DeviationReferencePage() {
  ...
  return <>
    ...
    <p className="mt-2 text-zinc-400">AP Toolbox Pro deviations for 4–8 deck games: 34 H17 plays, 32 S17 plays.</p>
    <Panel className="mt-7">
      <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.06] p-4 text-sm text-zinc-300">
        <p>Based exactly on the supplied AP Toolbox Pro charts for each dealer rule. Each catalog includes insurance, split, soft-total, hard-total, and late-surrender decisions. “Always” means the chart marks the surrender as a standing late-surrender play rather than a count threshold.</p>
