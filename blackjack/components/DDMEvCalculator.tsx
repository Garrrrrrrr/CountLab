      ...
      <div className="space-y-4">
        <Panel>
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[var(--accent)]">Removal-exact enumeration</p><h2 className="mt-2 text-xl font-semibold">Build the information state</h2></div>
            <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs text-[var(--accent)]">6-deck V1 · H17 · peek · dealer 22 pushes</span>
          </div>
        ...
        <Panel>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Player cards</h2><p className="mt-1 text-xs text-[var(--ink-muted)]">Order is irrelevant after the first draw; two-card A+T is recognized as blackjack.</p></div><GhostButton onClick={() => { invalidate(); setPlayer([]); }}>Clear hand</GhostButton></div>
          <div className="mt-4 flex min-h-14 flex-wrap gap-2 rounded-xl bg-well/20 p-3">
            {player.length ? player.map((rank, index) => <button type="button" key={`${rank}-${index}`} onClick={() => removePlayer(index)} className="pressable min-h-11 min-w-11 rounded-lg bg-white px-3 font-bold text-zinc-950" aria-label={`Remove ${rankLabel(rank)}`}>{rankLabel(rank)} ×</button>) : <span className="self-center text-sm text-[var(--ink-muted)]">Add a card below.</span>}
          </div>
          <div className="mt-3 grid grid-cols-5 gap-2 sm:grid-cols-10">{RANKS.map((rank) => <button type="button" key={rank} disabled={availableByRank[rank - 1] <= 0} onClick={() => addPlayer(rank)} className="pressable min-h-12 rounded-xl border border-overlay/10 bg-overlay/[.05] font-semibold hover:bg-overlay/[.1] disabled:opacity-20">{rankLabel(rank)}</button>)}</div>
          {value && <p className="mt-3 text-sm text-[var(--ink-muted)]">Current hand: <b className="text-[var(--ink)]">{value.bust ? `${value.hardTotal} bust` : `${value.soft ? "soft " : ""}${value.total}`}</b>{player.length === 1 && player[0] === 1 ? " · strict ace rule allows one final card only" : ""}</p>}
        </Panel>

        <Panel>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Previously exposed cards</h2><p className="mt-1 text-xs text-[var(--ink-muted)]">Enter discards from earlier rounds only. The current player cards and dealer upcard are removed automatically.</p></div><GhostButton onClick={() => { invalidate(); setDeadCards(Array(10).fill(0)); }}>Reset discards</GhostButton></div>
        ...
        {result ? (
          <>
            <Panel className="border border-emerald-400/20">
              <p className="text-xs font-bold uppercase tracking-wider text-[var(--accent)]">Exact recommendation</p>
              <h2 className="mt-2 text-3xl font-semibold">{ACTION_NAMES[result.action]}</h2>
              <p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">Optimal against the exact remaining rank composition, including optimal continuation after future hits and re-doubles.</p>
              {strategy && strategy.action !== result.action && <p className="mt-3 rounded-lg bg-amber-400/[.08] p-3 text-xs leading-5 text-[var(--warning)]">The practical Hi-Lo chart says {ACTION_NAMES[strategy.action]}, but full composition says {ACTION_NAMES[result.action]}. A one-parameter count cannot distinguish every composition with the same TC.</p>}
            </Panel>
            <Panel>
              <h2 className="font-semibold">Action EV</h2>
              <p className="mt-2 text-xs leading-5 text-[var(--ink-muted)]">Net EV is per current wager and conditioned on the dealer having completed an A/10 peek without blackjack.</p>
              <div className="mt-4 space-y-2">{calculatedActions.map((action) => {
                const ev = Number(result.actionEv[action]);
                return <div key={action} className={`rounded-xl border p-4 ${action === result.action ? "border-emerald-400/30 bg-emerald-400/[.07]" : "border-overlay/[.06] bg-well/20"}`}><div className="flex items-center justify-between gap-3"><b>{ACTION_NAMES[action]}</b><b className={ev >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{dollars(ev * currentWager)}</b></div><div className="mt-2 flex justify-between text-xs text-[var(--ink-muted)]"><span>{percent(ev)} of current wager</span>{action === "D" && <span>${(currentWager * 2).toFixed(2)} total at risk</span>}</div></div>;
              })}</div>
            </Panel>
            {result.prePeekActionEv && <Panel><h2 className="font-semibold">Peek and insurance</h2><p className="mt-3 text-sm text-[var(--ink-muted)]">Dealer blackjack probability before the peek: <b className="text-[var(--ink)]">{percent(result.dealerBlackjackProbability)}</b>.</p><div className="mt-4 space-y-2">{calculatedActions.map((action) => { const ev = Number(result.prePeekActionEv?.[action]); return <div key={action} className="flex items-center justify-between rounded-xl bg-well/20 px-4 py-3 text-sm"><span>{ACTION_NAMES[action]} main-hand EV before peek</span><b className={ev >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{dollars(ev * currentWager)}</b></div>; })}</div>{result.insuranceEvPerBaseUnit !== undefined && <div className="mt-4 rounded-xl border border-overlay/[.07] p-4"><div className="flex items-center justify-between gap-3"><b>Insurance</b><b className={result.insuranceEvPerBaseUnit >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{result.insuranceEvPerBaseUnit >= 0 ? "Take" : "Decline"}</b></div><p className="mt-2 text-xs text-[var(--ink-muted)]">EV {dollars(result.insuranceEvPerBaseUnit * baseBet)} on a ${(baseBet / 2).toFixed(2)} insurance stake · {percent(result.insuranceEvPerBaseUnit)} per base unit.</p></div>}</Panel>}
            <Panel><h2 className="font-semibold">Audit trail</h2><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><span className="text-xs text-[var(--ink-muted)]">Player states</span><b className="mt-1 block">{result.playerStates.toLocaleString()}</b></div><div><span className="text-xs text-[var(--ink-muted)]">Dealer states</span><b className="mt-1 block">{result.dealerStates.toLocaleString()}</b></div><div><span className="text-xs text-[var(--ink-muted)]">Runtime</span><b className="mt-1 block">{durationMs < 1000 ? `${durationMs.toFixed(0)} ms` : `${(durationMs / 1000).toFixed(2)} s`}</b></div><div><span className="text-xs text-[var(--ink-muted)]">Unseen cards</span><b className="mt-1 block">{result.unseenCards}</b></div></div></Panel>
          </>
        ) : (
          <Panel>
            <h2 className="font-semibold">What is calculated</h2>
            <p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">This is combinatorial enumeration, not simulation. Every draw removes its rank, every legal dealer hole card is integrated out, H17 is recursive, dealer 22 is separated from other busts, and the peek changes both hole-card and player-draw probabilities.</p>
            <p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">Version 1 blackjack pays the exact rank-model expectation of 1.625×: one-quarter suited at 2:1 and three-quarters unsuited at 3:2. No confidence interval is needed because there is no sampling error.</p>
          </Panel>
        )}
      </div>
    </div>
