...
export function UpdateToast({ hidden = false, onVisibilityChange }: { hidden?: boolean; onVisibilityChange?: (visible: boolean) => void }) {
  ...
  return (
    <div role="status" className="fixed inset-x-3 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-40 mx-auto flex max-w-md flex-wrap items-center justify-between gap-3 rounded-2xl border border-overlay/10 bg-[var(--paper-raised)] p-3 shadow-2xl backdrop-blur-2xl lg:bottom-[calc(1.5rem+env(safe-area-inset-bottom))]">
