...
export function AdminDirectory({ importPanel }: { importPanel?: ReactNode }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--accent)]">Admin</p><h1 className="mt-2 text-3xl font-semibold">Game directory</h1><p className="mt-2 text-sm text-[var(--ink-muted)]">Manage private draft locations and game offerings.</p></div><Link href="/directory" className="text-sm text-[var(--accent)] hover:underline">View private map →</Link></div>
    ...
    {error && <Panel><p role="alert" className="text-sm text-[var(--negative)]">{error}</p><GhostButton className="mt-3" onClick={() => { void reload(); if (selectedId) void listAdminGames(selectedId, true).then(setGames); }}>Reload records</GhostButton></Panel>}
    ...
    {tab === "import" ? importPanel : <div className="grid gap-5 lg:grid-cols-[minmax(16rem,22rem)_minmax(0,1fr)]">
      ...
      <div className="space-y-5">
        ...
        {selected && !editingLocation && <Panel>
          ...
          {!selected.deleted_at && <Link href={`/directory?location=${selected.id}`} className="mt-4 inline-block text-sm text-[var(--accent)] hover:underline">Open private view →</Link>}
        </Panel>}
        {selected && !editingLocation && <Panel>
