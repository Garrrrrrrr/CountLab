...
const DirectoryMap = dynamic(() => import("./DirectoryMap").then((module) => module.DirectoryMap), { ssr: false, loading: () => <div className="grid min-h-64 place-items-center rounded-xl border border-[var(--rule)]">Loading map…</div> });
...
function DirectoryContent() {
  ...
  return <div><div className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[var(--accent)]">Explore real tables</p><h1 className="mt-2 text-3xl font-semibold">Game directory</h1><p className="mt-2 max-w-2xl text-sm text-[var(--ink-muted)]">Find reported blackjack and Spanish 21 games by venue and rules. Reports describe conditions at a point in time.</p></div></div>
    ...
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3 text-sm text-[var(--ink-muted)]"><span role="status">{loading ? "Searching…" : `${results.total} location${results.total === 1 ? "" : "s"}`}</span><button type="button" className="text-[var(--accent)] underline" onClick={nearby}>Near me</button>{locationError && <span role="alert">{locationError}</span>}</div><div className="flex gap-2 lg:hidden"><button type="button" onClick={() => setView("list")} aria-pressed={view === "list"} className="min-h-11 rounded-lg border border-[var(--rule)] px-4">List</button><button type="button" onClick={() => setView("map")} aria-pressed={view === "map"} className="min-h-11 rounded-lg border border-[var(--rule)] px-4">Map</button></div></div>
