...
export function AdminNotesHistory({ locationId, games }: { locationId: string; games: DirectoryGame[] }) {
  ...
  return <Panel>
    ...
    <div className="mt-4 space-y-2">{notes.map((note) => <div key={note.id} className="rounded-lg border border-[var(--rule)] p-3"><div className="flex flex-wrap justify-between gap-2"><p className="text-xs font-semibold uppercase tracking-wide text-[var(--ink-muted)]">{note.audience === "admin" ? "Admin only" : "Public"} · {note.category}</p><div className="flex gap-2"><button className="text-xs text-[var(--accent)] hover:underline" onClick={() => edit(note)}>Edit</button><button className="text-xs text-[var(--negative)] hover:underline" onClick={() => setDeleting(note)}>Delete</button></div></div><p className="mt-2 whitespace-pre-wrap text-sm">{note.body}</p></div>)}{notes.length === 0 && <p className="text-sm text-[var(--ink-muted)]">No notes yet.</p>}</div>
