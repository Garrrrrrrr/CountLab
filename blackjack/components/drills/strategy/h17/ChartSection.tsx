 ...
 */
export const ChartSection = memo(function ChartSection({ section, values, indexOf, selectedKey, mode, keypad, rows, filled, total, header, footer, onKeyDown, onSelect, register }: SectionProps) {
  ...
  return (
    <Panel aria-labelledby={`h17-${section.id}-title`} className="scroll-mt-40">
      ...
      <div className="relative">
        <div ref={measure} onScroll={(event) => measure(event.currentTarget)} data-testid={`h17-rail-${section.id}`} className="-mx-1 snap-x snap-mandatory overflow-x-auto scroll-pl-11 px-1">
                    ...
                    return (
                      <td key={dealer} className="snap-start scroll-ml-12 align-top [scroll-margin-bottom:calc(var(--dock-clearance,0px)+4.75rem+env(safe-area-inset-bottom))] [scroll-margin-top:5rem] sm:[scroll-margin-top:9.5rem] lg:[scroll-margin-bottom:1rem]">
                        <input
                          ...
                          aria-label={label}
                          aria-describedby={settled ? statusId : undefined}
