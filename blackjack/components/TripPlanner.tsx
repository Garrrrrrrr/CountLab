...
import type { Bankroll } from "@/lib/blackjack/journal";
...
export function TripPlanner() {
  const [savedBankrolls, setSavedBankrolls] = useState<Bankroll[]>([]);
  const [bankrollId, setBankrollId] = useState("");
  const [useIndices, setUseIndices] = useState(true);
  const [handsSchedule, setHandsSchedule] = useState<HandCountPoint[] | undefined>();
  ...
  useEffect(() => { const refresh = () => setSavedBankrolls(journalLibrary.bankrolls().filter((entry) => !entry.archived)); refresh(); addEventListener(journalLibrary.event, refresh); return () => removeEventListener(journalLibrary.event, refresh); }, []);
  ...
  const rules = useMemo(() => ({ ...DEFAULT_ADVANTAGE_RULES, decks, penetration: dealt / decks, dealerHitsSoft17, doubleAfterSplit, resplitAces, lateSurrender, blackjackPayout, useIndices }), [useIndices, blackjackPayout, decks, dealt, dealerHitsSoft17, doubleAfterSplit, resplitAces, lateSurrender]);
  const ruleFlags = useMemo(() => ruleAdjustmentFlagsFromRules(rules), [rules]);
  const plan = useMemo(
    () => planTrip({ bankroll, bettingUnit, playerHands, handsPerHour, hours: tripHours, rules, ramp, handsByTrueCount: handsSchedule }),
    [bankroll, bettingUnit, playerHands, handsPerHour, tripHours, rules, ramp, handsSchedule],
  );
  ...
  const useJournalBankroll = () => {
    ...
    const total = currentBankroll(journalLibrary.sessions().filter((entry) => entry.bankrollId === bankrollId), journalLibrary.transactions().filter((entry) => entry.bankrollId === bankrollId));
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <Section title="Trip setup" summary={`${decks}D · ${money(bankroll)} bankroll · ${tripHours} hours`} icon="fa-suitcase" collapseOnMobile>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-end">
              <div className="w-full space-y-2"><Select label="Journal bankroll" value={bankrollId} onChange={(event) => setBankrollId(event.target.value)}><option value="">Choose a bankroll</option>{savedBankrolls.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</Select><GhostButton className="w-full" disabled={!bankrollId} onClick={useJournalBankroll}>Use selected bankroll</GhostButton></div>
            </div>
            <NumberField label="Betting unit" value={bettingUnit} min={0.01} prefix="$" onValueChange={setBettingUnit} />
            <Select label="Simultaneous hands" value={playerHands} onChange={(event) => { setPlayerHands(Number(event.target.value)); setHandsSchedule(undefined); }}>{[1, 2, 3].map((value) => <option key={value} value={value}>{value} hand{value === 1 ? "" : "s"}</option>)}</Select>
          ...
          {handsSchedule && <p className="mt-3 text-sm text-[var(--ink-muted)]">Using the saved per-count hand schedule. Changing simultaneous hands replaces it with a constant count.</p>}
          <Switch label="Use index deviations" checked={useIndices} onChange={setUseIndices} />
          ...
          <div className="mt-4">
            <div className="mb-2 flex items-center justify-between"><p className="text-[.8rem] font-medium text-[var(--ink-muted)]">Ramp</p><div className="w-40"><Select label="" aria-label="Ramp preset" defaultValue="1-8" onChange={(event) => RAMPS[event.target.value] && setRamp(expandRamp(RAMPS[event.target.value]))}>{Object.keys(RAMPS).map((name) => <option key={name}>{name}</option>)}</Select></div></div>
        ...
        <div className="space-y-5">
          ...
          <Panel>
            ...
            <p className="mt-1 text-xs text-[var(--ink-muted)]">Probability the trip ends down at least this much.</p>
          ...
          <Panel>
            ...
            <p className="mt-1 text-xs text-[var(--ink-muted)]">Probability the trip reaches this net win at some point.</p>
            <div className="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
