    ...
    <div className="mt-5 space-y-4">
      <Panel>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[var(--accent)]">Simulation-backed scenario builder</p><h2 className="mt-2 text-xl font-semibold">Game, bankroll, and pace</h2></div>
          <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs text-[var(--accent)]">One occupied spot · six decks · V1 H17</span>
        </div>
        ...
        <p className="mt-4 text-sm leading-6 text-[var(--ink-muted)]">Only independently simulated combinations are selectable. Changing penetration resets deck estimation and strategy to the audited exact-TC, full-index package.</p>
      </Panel>

      <Panel>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[var(--info)]">Bet spread</p><h2 className="mt-2 text-lg font-semibold">Wager at every true count</h2><p className="mt-2 max-w-2xl text-sm text-[var(--ink-muted)]">Enter unit multiples. A zero wager watches that count without playing it; hourly results still use observed table rounds.</p></div>
              ...
              {profile.buckets.map((bucket) => (
                <div key={bucket.tc} className="rounded-xl border border-overlay/[.07] bg-overlay/[.02] p-3">
                  ...
                  <div className="mt-2.5 flex items-center gap-2">
                    ...
                    <button type="button" aria-label={`Zero bet at true count ${bucket.label}`} disabled={bucket.units === 0} onClick={() => changeBet(bucket.tc, 0)} className="min-h-11 shrink-0 rounded-lg border border-red-400/20 bg-red-400/[.06] px-3 text-xs font-semibold text-[var(--negative)] hover:bg-red-400/[.12] disabled:opacity-30">Zero</button>
                  </div>
                  <div className="mt-2 flex items-center justify-between text-xs text-[var(--ink-muted)]"><span>{money(bucket.units * unit)} wager</span><span className={bucket.evContribution >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{signedMoney(bucket.evContribution * unit, 3)} / observed round</span></div>
                </div>
              ))}
            </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Panel>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div><p className="text-xs text-[var(--ink-muted)]">Expected result</p><p className={`mt-2 text-lg font-semibold ${result.tripEv >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}`}>{signedMoney(result.tripEv)}</p></div>
            <div><p className="text-xs text-[var(--ink-muted)]">Standard deviation</p><p className="mt-2 text-lg font-semibold">{money(result.tripSd)}</p></div>
      ...
      <Panel>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Scenario audit</h2><p className="mt-2 text-sm text-[var(--ink-muted)]">{profile.description}</p></div><span className={`rounded-full px-3 py-1 text-xs ${Math.abs(profile.evPerRound) > profile.ci95 ? "bg-emerald-400/10 text-[var(--accent)]" : "bg-amber-400/10 text-[var(--warning)]"}`}>{Math.abs(profile.evPerRound) > profile.ci95 ? "EV exceeds simulation CI" : "EV overlaps simulation noise"}</span></div>
        <dl className="mt-5 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3 xl:grid-cols-6">
          <div><dt className="text-xs text-[var(--ink-muted)]">Rounds sampled</dt><dd className="mt-1 font-semibold">{profile.rounds.toLocaleString()}</dd></div>
          <div><dt className="text-xs text-[var(--ink-muted)]">Penetration</dt><dd className="mt-1 font-semibold">{profile.cutDecks === null ? "CSM" : `${6 - profile.cutDecks} decks dealt`}</dd></div>
          <div><dt className="text-xs text-[var(--ink-muted)]">Count</dt><dd className="mt-1 font-semibold">{profile.count}</dd></div>
          <div><dt className="text-xs text-[var(--ink-muted)]">Policy</dt><dd className="mt-1 font-semibold">{profile.policy}</dd></div>
          <div><dt className="text-xs text-[var(--ink-muted)]">EV / unit / round</dt><dd className="mt-1 font-semibold">{profile.evPerRound >= 0 ? "+" : ""}{profile.evPerRound.toFixed(6)}</dd></div>
          <div><dt className="text-xs text-[var(--ink-muted)]">Source</dt><dd className="mt-1 truncate font-semibold" title={profile.source}>{sourceFile(profile.source)}</dd></div>
        </dl>
        <p className="mt-5 text-xs leading-5 text-[var(--ink-muted)]">Mean and second moment are recomputed from the selected simulation&apos;s composition-conditioned TC buckets. Wagers scale outcome means linearly and second moments quadratically, including insurance and every redouble. No blackjack coefficients or penetration interpolation are used.</p>
      </Panel>
    </div>
  );
}
