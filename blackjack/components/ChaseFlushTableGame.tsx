  ...
  return (
    <div className="mt-5 grid gap-5 pb-24 lg:pb-0 xl:grid-cols-[1fr_320px]">
      <div className="space-y-5">
        ...
        <CasinoTable>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--accent)]/55">Chase the Flush</p><p className="mt-1 text-sm text-emerald-50/80">Bankroll <b className="text-white">${bankroll.toFixed(2)}</b></p></div><span className="rounded-full bg-black/20 px-3 py-1 text-xs text-[var(--accent)]/70">{phase === "betting" ? "Place bets" : phase === "result" ? "Round complete" : revealDealer ? "Dealer card exposed" : "Decision in progress"}</span></div>
      ...
      <div className="space-y-5">
        ...
        <Panel><h2 className="font-semibold">Session</h2><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-black/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Rounds</p><b>{stats.rounds}</b></div><div className="rounded-xl bg-black/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Wins</p><b>{stats.wins}</b></div><div className="rounded-xl bg-black/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Net</p><b className={stats.net >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{stats.net >= 0 ? "+" : ""}${money(stats.net)}</b></div></div></Panel>
