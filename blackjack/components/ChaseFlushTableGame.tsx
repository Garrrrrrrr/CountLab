...
export function ChaseFlushTableGame({
  ...
  const finish = (multiplier: number, folded = false) => {
    ...
    setHistory((rows) => [{ id: Date.now(), result, net, bankroll: nextBankroll, detail: `${flushName(playerCards)} · Ante ${breakdown.ante >= 0 ? "+" : ""}${money(breakdown.ante)} · X-Tra ${breakdown.xtra >= 0 ? "+" : ""}${money(breakdown.xtra)} · All-In ${breakdown.allIn >= 0 ? "+" : ""}${money(breakdown.allIn)}` }, ...rows]);
  ...
  const actionButtons = <>
    ...
    {phase === "river" && <><Button onClick={() => { judge("BET", "river"); finish(1); }}>Bet 1x · ${money(ante)}</Button><GhostButton onClick={() => { judge("FOLD", "river"); finish(0, true); }} className="text-[var(--negative)]">Fold</GhostButton></>}
  ...
  return (
    <div className="mt-5 grid grid-cols-1 gap-5 pb-24 lg:pb-0 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-5">
        ...
        <CasinoTable>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[var(--accent)]/55">Chase the Flush</p><p className="mt-1 text-sm text-emerald-50/80">Bankroll <b className="text-white">${bankroll.toFixed(2)}</b></p></div><span className="rounded-full bg-well/20 px-3 py-1 text-xs text-[var(--accent)]/70">{phase === "betting" ? "Place bets" : phase === "result" ? "Round complete" : revealDealer ? "Dealer card exposed" : "Decision in progress"}</span></div>
          ...
          <div aria-live="polite" className="mx-auto mt-5 max-w-2xl rounded-xl bg-well/25 p-3 text-center text-sm text-emerald-50/80">{message}</div>
        ...
        <Panel><h2 className="mb-4 font-semibold">Chip rack</h2><ChipRack selected={selectedChip} onSelect={setSelectedChip} disabled={locked} /><p className="mt-4 text-center text-xs text-[var(--ink-muted)]">Select a chip, then tap Ante. X-Tra automatically matches it.</p></Panel>
      </div>
      <div className="space-y-5">
        ...
        <Panel><h2 className="font-semibold">Session</h2><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Rounds</p><b>{stats.rounds}</b></div><div className="rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Wins</p><b>{stats.wins}</b></div><div className="rounded-xl bg-well/20 p-3"><p className="text-xs text-[var(--ink-muted)]">Net</p><b className={stats.net >= 0 ? "text-[var(--accent)]" : "text-[var(--negative)]"}>{stats.net >= 0 ? "+" : ""}${money(stats.net)}</b></div></div></Panel>
        <Panel><h2 className="mb-4 font-semibold">Round history</h2><GameHistory rows={history} /></Panel>
        <Panel><h2 className="font-semibold">Table rules</h2><ul className="mt-3 space-y-2 text-xs leading-5 text-[var(--ink-muted)]"><li>• Ante and X-Tra are equal mandatory bets.</li><li>• Bet 3x opening, 2x after two board cards, or 1x after all four.</li><li>• Only flush length and suited ranks count; ordinary poker combinations do not.</li><li>• Dealer qualifies with a four-card flush or 9-high three-card flush.</li><li>• X-Tra pays 1/5/{sixCardPayout}/250 for four through seven suited cards.</li></ul></Panel>
      </div>
